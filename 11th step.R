...
load('rdata/step_8_train_phe.Rdata')
...
uncox <- uncoxf(indat = train_phe,ingene = invarlist)
uncox
mucox <- mucoxf(indat = train_phe,ingene = invarlist)
mucox
...
TCGA_clinc <- train_phe[order(train_phe$RiskScore),]
...
ann_col = data.frame(Age        = TCGA_clinc$Age2,
